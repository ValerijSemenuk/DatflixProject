...
class ReviewService(private val reviewRepository: ReviewRepository) {
    ...
    suspend fun getReviewById(id: Int): Review? = reviewRepository.getById(id)

    suspend fun createReview(review: Review): Review {
        require(review.text.isNotBlank()) { "Review text cannot be empty" }
        require(review.rating in 1..10) { "Rating must be between 1 and 10" }
        return reviewRepository.create(review)
    }

    suspend fun updateReview(review: Review): Boolean {
        val existing = reviewRepository.getById(review.id) ?: return false
